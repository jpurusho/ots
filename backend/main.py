...
def _get_setting_json(key: str):
    """Get a JSON setting value, parsed."""
...
def _build_card_html(offering: dict, church_name: str, accent: str = "#4f46e5") -> str:
    """Build email-safe HTML card for an offering (matches frontend buildEmailCard)."""
    cats = [
        ("general", "General"),
        ("cash", "Cash"),
    ...
    total = sum(offering.get(k, 0) or 0 for k, _ in cats)
    date_str = offering.get("offering_date") or "Unknown"
    ...
    for key, label in cats:
        amt = offering.get(key, 0) or 0
        if amt > 0:
            rows += (
                '<tr>'
                f'<td style="padding:9px 20px;font-size:13px;color:#6b7280;border-bottom:1px solid #f1f5f9">{label}</td>'
                f'<td style="padding:9px 20px;font-size:13px;text-align:right;border-bottom:1px solid #f1f5f9;font-weight:600;color:#111827">${amt:.2f}</td>'
                '</tr>'
            )
    ...
    return (
        ...
        + rows +
        '<tr>'
        f'<td style="padding:12px 20px;font-size:14px;font-weight:700;color:{accent};border-top:2px solid {accent}">Total</td>'
        f'<td style="padding:12px 20px;font-size:14px;font-weight:700;color:{accent};text-align:right;border-top:2px solid {accent}">${total:.2f}</td>'
        '</tr>'
        '</table>'
        '</td></tr>'
        '</table>'
        f'<p style="margin:10px 0 0;font-size:10px;color:#94a3b8;text-align:center">Generated by OTS · {today}</p>'
        '</div>'
    )


def _send_auto_email(offering: dict, church_name: str, email_group: dict, accent: str):
    """Send the HTML card email for an offering to the configured group."""
    smtp_user = _get_setting("smtp_user")
    smtp_pass = _get_setting("smtp_password")
    if not smtp_user or not smtp_pass:
        return {"success": False, "error": "SMTP not configured"}

    html_body = _build_card_html(offering, church_name, accent)

    date_str = offering.get("offering_date") or ""
    if date_str:
        try:
            dt = datetime.strptime(date_str, "%Y-%m-%d")
            date_str = dt.strftime("%-m/%d/%Y")
    ...
    subject = email_group.get("subject", "")
    if subject:
        subject = subject.replace("{church}", church_name).replace("{period}", f"Week of {date_str}")
    else:
        subject = f"{church_name} — Week of {date_str}"

    to_list = [e.strip() for e in email_group.get("to", "").split(",") if e.strip()]
    cc_list = [e.strip() for e in email_group.get("cc", "").split(",") if e.strip()]
    bcc_list = [e.strip() for e in email_group.get("bcc", "").split(",") if e.strip()]

    if not to_list:
        return {"success": False, "error": "No recipients in email group"}
    ...
    try:
        ...
        return {"success": True, "recipients": len(to_list)}
    except Exception as e:
        return {"success": False, "error": str(e)}


def _check_auto_approve(offering: dict) -> bool:
    """Check if an offering meets the confidence threshold for auto-approval."""
    min_total = float(_get_setting("auto_approve_min_total") or "0")
    require_date = _get_setting("auto_approve_require_date") == "true"

    total = sum(offering.get(k, 0) or 0 for k in ["general", "cash", "sunday_school", "building_fund", "misc"])
    if total <= min_total:
        return False

    if require_date and not offering.get("offering_date"):
        return False

    scan_data = offering.get("scan_data")
...
def _get_offerings_for_week(offering_date: str) -> list:
    """Get all offerings for the same week (Sunday) as the given date."""
    try:
        dt = datetime.strptime(offering_date, "%Y-%m-%d")
    except ValueError:
        return []
    sunday = dt - timedelta(days=dt.weekday() + 1) if dt.weekday() != 6 else dt
    week_start = sunday.strftime("%Y-%m-%d")
    week_end = (sunday + timedelta(days=6)).strftime("%Y-%m-%d")
    result = supabase.table("offerings").select("*").gte("offering_date", week_start).lte("offering_date", week_end).execute()
    return result.data or []


@app.post("/api/automation/auto-email")
async def auto_email_offering(req: AutoEmailRequest):
    """Send auto-email for a single approved offering (called after manual/auto approval)."""
    auto_enabled = _get_setting("auto_email_enabled") == "true"
    if not auto_enabled:
        return {"success": False, "skipped": True, "reason": "Auto-email disabled"}

    mode = _get_setting("auto_email_mode") or "per_approval"
    group_id = _get_setting("auto_email_group_id") or ""
    accent = _get_setting("card_accent_color") or "#4f46e5"
    church_name = _get_setting("church_name") or "Church"

    email_groups = _get_setting_json("email_groups") or []
    group = next((g for g in email_groups if g.get("id") == group_id), None)
    if not group:
        return {"success": False, "error": "Configured email group not found"}
    ...
    if offering.get("status") != "approved":
        return {"success": False, "error": "Offering not approved"}

    if mode == "per_approval":
        email_result = _send_auto_email(offering, church_name, group, accent)
        return email_result

    elif mode == "weekly_batch":
        if not offering.get("offering_date"):
            return {"success": False, "error": "No offering date for weekly check"}

        week_offerings = _get_offerings_for_week(offering["offering_date"])
        all_approved = all(o.get("status") == "approved" for o in week_offerings)
        ...
        for o in sorted(week_offerings, key=lambda x: x.get("offering_date") or ""):
            combined_html += _build_card_html(o, church_name, accent) + '<br style="margin:16px 0">'

        date_str = offering["offering_date"]
        try:
            dt = datetime.strptime(date_str, "%Y-%m-%d")
            date_str = dt.strftime("%-m/%d/%Y")
        except ValueError:
        ...
        to_list = [e.strip() for e in group.get("to", "").split(",") if e.strip()]
        cc_list = [e.strip() for e in group.get("cc", "").split(",") if e.strip()]
        bcc_list = [e.strip() for e in group.get("bcc", "").split(",") if e.strip()]
...
async def run_pipeline(req: RunPipelineRequest):
    ...
    1. Import from Google Drive (with auto-scan)
    2. Auto-approve offerings meeting confidence threshold
    3. Send email for approved offerings
    """
    church_name = _get_setting("church_name") or "Church"
    accent = _get_setting("card_accent_color") or "#4f46e5"
    auto_approve_enabled = _get_setting("auto_approve_enabled") == "true"
    auto_email_enabled = _get_setting("auto_email_enabled") == "true"
    mode = _get_setting("auto_email_mode") or "per_approval"
    group_id = _get_setting("auto_email_group_id") or ""
    ...
    try:
        import_result = await import_from_drive(DriveImportRequest(auto_scan=True))
        summary["imported"] = import_result.get("imported", 0)
        summary["scanned"] = sum(
            1 for r in import_result.get("results", [])
            if r.get("scan", {}).get("success")
        )
    ...
    if req.auto_approve and auto_approve_enabled:
        scanned = supabase.table("offerings").select("*").eq("status", "scanned").execute()
        for offering in (scanned.data or []):
            if _check_auto_approve(offering):
                try:
                    supabase.table("offerings").update({
                        "status": "approved",
                        "locked": 1,
                        "locked_at": datetime.utcnow().isoformat(),
                        "approved_by_email": "automation",
                    ...
                    if req.auto_email and auto_email_enabled and group and mode == "per_approval":
                        ...
                        if email_result.get("success"):
                            ...
                            summary["errors"].append(f"Email for #{offering['id']}: {email_result.get('error')}")
                except Exception as e:
    ...
    if req.auto_email and auto_email_enabled and group and mode == "weekly_batch":
        ...
        for o in (approved_today.data or []):
            ...
            if all(wo.get("status") == "approved" for wo in week_offerings) and week_offerings:
                # Build combined card
                combined_html = ""
                for wo in sorted(week_offerings, key=lambda x: x.get("offering_date") or ""):
                    combined_html += _build_card_html(wo, church_name, accent) + '<br style="margin:16px 0">'
                ...
                subject = group.get("subject", "")
                if subject:
                    subject = subject.replace("{church}", church_name).replace("{period}", f"Week of {date_label}")
                else:
                    subject = f"{church_name} — Week of {date_label}"
